...
const Employers = () => {
  ...
  useEffect(() => {
    const fetchEmployers = async () => {
      try {
        const res = await axios.get("http://localhost:2022/api/user/getallemployer", {
  ...
  const handleDelete = async (userId) => {
    try {
      ...
      setEmployers(employers.filter((employer) => employer._id !== userId));
    } catch (error) {
      console.error(error);
    }
  };
  ...
  return (
    <section className="jobs page">
      <div className="container">
        <h1>ALL AVAILABLE Recruiters</h1>
